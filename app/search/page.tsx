...
import { langchainEmbeddings } from "@/lib/langchain-vectorize";
...
const Page = async ({ searchParams }: PageProps) => {
  ...
  const products: CoreProduct[] = await db
    ...
    .from(productsTable)
    .where(
      sql`to_tsvector('simple', lower(${productsTable.name} || ' ' || ${productsTable.description})) @@ to_tsquery('simple', lower(${query.split(" ").join(" & ")}))`,
    );

  if (products.length < 3) {
    const vector = await langchainEmbeddings(query);
